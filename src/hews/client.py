...
class HNClient:
    ...
    async def fetch_stories(self, section: str, limit: int = 30) -> List[Story]:
        ...
        if section not in self.SECTION_ENDPOINTS:
            ...
            raise HNClientError(f"Invalid section '{section}'. Valid sections: {valid_sections}")
        ...
        try:
            ...
            raise HNClientError(f"HTTP error fetching {section} stories: {e.response.status_code}") from e
        except httpx.RequestError as e:
            raise HNClientError(f"Network error fetching {section} stories: {e}") from e
        except Exception as e:
            raise HNClientError(f"Unexpected error fetching {section} stories: {e}") from e
    ...
    async def fetch_item(self, item_id: int) -> Union[Story, Comment]:
        ...
        try:
            ...
            raise HNClientError(f"HTTP error fetching item {item_id}: {e.response.status_code}") from e
        except httpx.RequestError as e:
    ...
    async def _fetch_items_concurrent(self, item_ids: List[int]) -> List[Union[Story, Comment]]:
